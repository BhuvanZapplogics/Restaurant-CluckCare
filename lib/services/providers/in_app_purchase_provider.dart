...
class InAppPurchaseProvider extends ChangeNotifier {
  ...
  Future<void> setupStoreData() async {
    ...
    // Filter out any empty/invalid product IDs to avoid bad queries in release/profile
    final validIds = _itemIdentifiers.where((id) => id.isNotEmpty).toSet();
    if (validIds.isEmpty) {
      // No valid product IDs configured – keep service active but no products
      _products = [];
      notifyListeners();
      return;
    }
    ...
    final ProductDetailsResponse productResponse =
        await _paymentService.queryProductDetails(validIds);
